...
function renderProfile(a) {
  ...
  // ── Server-Side HTTP Fingerprint (catches terminal attacks) ──
  const sfpEl = $('pSFP');
  if (sfpEl) {
    sfpEl.textContent = a.sfpId || '—';
  }
  const clientTypeEl = $('pClientType');
  if (clientTypeEl) {
    if (a.isToolAttack) {
      clientTypeEl.innerHTML = '<span style="color:#ef4444;font-weight:bold">⚠️ TERMINAL / SCRIPT</span>';
    } else if (a.sfpBrowser === true) {
      clientTypeEl.innerHTML = '<span style="color:#10b981">🌐 Browser</span>';
    } else if (a.sfpHeaders != null) {
      clientTypeEl.innerHTML = `<span style="color:#eab308">❓ Unknown (${a.sfpHeaders} headers)</span>`;
    } else {
...
async function blockCurrentSFP() {
  const a = allAttackers.find(x => x.session === selectedSession);
  if (!a || !a.sfpId) {
    showToast("⚠️ No HTTP fingerprint available for this attacker", "orange");
    return;
  }
  try {
    ...
    if (data.ok) {
      ...
      updateBlockBtn(a);
      showToast('🖥️ Tool signature blocked! Terminal attacks from this client will be rejected.', 'red');
    }
  } catch (e) { console.error(e); }
}
...
async function unblockCurrentSFP() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      updateBlockBtn(a);
      showToast('✅ Tool signature unblocked', 'green');
    }
  } catch (e) { console.error(e); }
}
...
function updateBlockBtn(a) {
  ...
  const bT = $('blockSFPBtn');
  const uT = $('unblockSFPBtn');

  const sB = blockedSessionSet.has(a.session);
  const fB = a.fpId && blockedFPSet.has(a.fpId);
  const tB = a.sfpId && blockedSFPSet.has(a.sfpId);
  ...
  if (bT) { bT.style.display = tB ? 'none' : 'block'; uT.style.display = tB ? 'block' : 'none'; }
}
...
function renderBlockedList() {
  ...
  // 4. Server Fingerprints (Tool signatures)
...
function bindButtons() {
  ...
  if (bT) bT.onclick = blockCurrentSFP;
  if (uT) uT.onclick = unblockCurrentSFP;
}
