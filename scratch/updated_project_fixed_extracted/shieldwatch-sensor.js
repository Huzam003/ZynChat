...
// Every HTTP client (curl, Python, wget, browser) sends headers in a unique
// order with unique presence patterns. This fingerprint works WITHOUT JavaScript
// — it identifies the TOOL, not the device, so even terminal attacks are tracked.
const blockedServerFPs = new Set();
...
setInterval(fetchServerFPBlocklist, 30_000);
...
function httpMiddleware(req, res, next) {
  ...
  // ── Session block (surgical — kicks one user without affecting others) ──
  const sessionUser = req.session?.username;
  const sessionKey  = sessionUser || `anon@${reqIP}`;
  if (blockedSessions.has(sessionUser) || blockedSessions.has(sessionKey)) {
    console.log(`[ShieldWatch] ✂️ BLOCKED SESSION: ${sessionKey} | IP: ${reqIP} | path: ${rawPath}`);
    return res.status(403).json({
      ok: false, blocked: true,
...
// Forces clients to solve a SHA-256 partial preimage before accessing sensitive
// endpoints. Browsers solve it in JavaScript; terminal tools must implement it
// manually. Makes automated brute-force and scraping EXPENSIVE.
//
// Flow:
//   1. Client hits /api/pow/challenge → gets { challenge, difficulty }
//   2. Client finds a `nonce` such that SHA-256(challenge + nonce) starts with
//      `difficulty` zero bits
//   3. Client sends the nonce in the x-pow-nonce header on subsequent requests
//   4. Sensor verifies the proof before allowing the request through
const powChallenges = new Map(); // sessionId → { challenge, difficulty, issued }
const POW_DIFFICULTY = 4;        // 4 hex zeros = ~65K hashes ≈ 50-200ms on a browser
const POW_EXPIRY_MS  = 5 * 60 * 1000; // 5 minutes
...
setInterval(() => {
  const now = Date.now();
  for (const [sid, data] of powChallenges) {
    if (now - data.issued > POW_EXPIRY_MS) powChallenges.delete(sid);
  }
}, 60_000);
