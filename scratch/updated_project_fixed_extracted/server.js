...
// ─────────────────────────────────────────────────────────────────────────────
// Proof-of-Work Challenge API
// Makes automated attacks computationally expensive. Browsers solve it in JS;
// terminal tools must implement SHA-256 mining manually.
// ─────────────────────────────────────────────────────────────────────────────
app.get('/api/pow/challenge', (req, res) => {
  if (!sw || !sw.generateChallenge) {
    return res.json({ ok: false, error: 'ShieldWatch not active' });
  }
  const challenge = sw.generateChallenge();
  const sid = req.sessionID || 'anon';
  sw.powChallenges.set(sid, { challenge, difficulty: sw.POW_DIFFICULTY, issued: Date.now() });
  res.json({ ok: true, challenge, difficulty: sw.POW_DIFFICULTY });
});
...
app.post('/api/pow/verify', (req, res) => {
  ...
  const sid = req.sessionID || 'anon';
  const data = sw.powChallenges.get(sid);
  if (!data) return res.status(400).json({ ok: false, error: 'No active challenge' });

  const { nonce } = req.body;
  if (!nonce) return res.status(400).json({ ok: false, error: 'Nonce required' });

  if (sw.verifyPoW(data.challenge, String(nonce), data.difficulty)) {
    req.session.powVerified = Date.now();
    sw.powChallenges.delete(sid);
    res.json({ ok: true, verified: true });
  } else {
    res.status(403).json({ ok: false, error: 'Invalid proof-of-work' });
  }
});
