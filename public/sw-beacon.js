    ...
    setTimeout(function () {
      ...
      try {
        ...
        if (AudioCtx) {
          ...
          fp.audioInfo = [
            actx.sampleRate,
            actx.destination.channelCount,
            actx.destination.maxChannelCount
          ].join(',');
          actx.close();
        } else {
          fp.audioInfo = '';
      ...
      // WebGL Shader Highp Float Precision
      try {
        var canvas2 = document.createElement('canvas');
        var gl2 = canvas2.getContext('webgl') || canvas2.getContext('experimental-webgl');
        if (gl2) {
          var highp = gl2.getShaderPrecisionFormat(gl2.FRAGMENT_SHADER, gl2.HIGH_FLOAT);
          fp.webglPrecision = highp ? [highp.rangeMin, highp.rangeMax, highp.precision].join(',') : '';
        } else {
          fp.webglPrecision = '';
        }
      } catch (e) {
      ...
      // ── Stable device ID — hardware & forensic signals ───────────────────
      // Combines platform, CPU cores, RAM, screen, GPU, and forensic signals into one hash.
