# ZynChat + ShieldWatch — Demo Setup Guide
...
### Step 1 — Deploy ZynChat to Render

1. Go to https://github.com and create a new repository called `zynchat`
2. Open Terminal and run:
   ```bash
   cd ~/Desktop/ZynChat
   git init
   git add .
   git commit -m "ZynChat initial deploy"
   git remote add origin https://github.com/YOUR_USERNAME/zynchat.git
...
4. Go to Render dashboard → Your zynchat service → Environment
...
### Browser 2 — ZynChat (the target — share this URL with class):
```
https://zynchat-kuuz.onrender.com
```
...
1. Log into ZynChat as **alice**
2. In the SAME browser, open a new tab and go to:
   ```
   https://zynchat-kuuz.onrender.com/csrf-attack.html
   ```
...
| Problem | Fix |
|---------|-----|
| ZynChat not loading | Check Render dashboard — may be sleeping (free tier sleeps after 15 min inactivity, first load takes ~30 sec) |
